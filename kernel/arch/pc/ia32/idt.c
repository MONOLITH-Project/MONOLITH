...
void idt_set_gate(uint8_t num, void *handler, uint8_t flags)
{
    ...
    _idt_entries[num].offset0 = address & 0xFFFF;
    _idt_entries[num].selector = 0x08;
    _idt_entries[num].zero = 0;
    _idt_entries[num].flags = flags;
    _idt_entries[num].offset1 = (address >> 16) & 0xFFFF;
}
...
void idt_init()
{
    debug_log("Initializing the IA-32 IDT...\n");
    _idtr.limit = sizeof(_idt_entries) - 1;
    _idtr.base = (uint32_t) (uintptr_t) &_idt_entries;
...
void isr_handler(struct interrupt_registers *regs)
{
    if (regs->isr_number < 32) {
        ...
        case 14: {
            ...
            itohex(address, error_message + strlen("Page fault at 0x"));
        } break;
        case 13: {
            uintptr_t address = regs->rip;
            strcpy(error_message, "General Protection Fault at 0x");
            itohex(address, error_message + strlen("General Protection Fault at 0x"));
        } break;
        default:
...
{
    _irq_routines[num] = handler;
    interrupts_set_irq_mask(num, false);
}

void irq_unregister_handler(uint8_t num)
{
    _irq_routines[num] = NULL;
    interrupts_set_irq_mask(num, true);
}
